...
const socials = [
  ...
  {label:"Email",    handle:"aberrada@et.esiea.fr", url:"mailto:aberrada@et.esiea.fr", icon:"@"},
];
...
export default function Contact() {
  ...
  const copyEmail = () => {
    navigator.clipboard.writeText("aberrada@et.esiea.fr");
    setCopied(true); setTimeout(()=>setCopied(false), 2000);
  };

  return (
    <section id="contact" ref={ref} style={{ background:"var(--bg-2)", borderTop:"1px solid var(--border)", padding:"130px 0 80px" }}>
      <div style={{ maxWidth:"1140px", margin:"0 auto", padding:"0 48px" }}>
        ...
        <div className="contact-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"88px", alignItems:"start" }}>
          {/* LEFT */}
          <div>
            <h2 className="fade-in font-display" style={{ fontSize:"clamp(34px, 5vw, 52px)", lineHeight:1.08, letterSpacing:"-0.025em", marginBottom:"24px" }}>
            ...
            <div className="fade-in" style={{marginBottom:"32px"}}>
              <button onClick={copyEmail} style={{
                display:"flex", alignItems:"center", gap:"12px", background:"var(--bg-3)",
                border:"1px solid var(--border)", borderRadius:"6px", padding:"14px 18px",
                cursor:"pointer", width:"100%", transition:"border-color 0.2s",
              }}
              onMouseOver={e=>e.currentTarget.style.borderColor="var(--accent)"}
              onMouseOut={e=>e.currentTarget.style.borderColor="var(--border)"}>
                <span style={{fontFamily:"var(--font-mono)", fontSize:"13px", color:"var(--text)", flex:1, textAlign:"left"}}>aberrada@et.esiea.fr</span>
            ...
            <div className="fade-in" style={{display:"flex", flexDirection:"column", gap:"10px"}}>
              {socials.map(s=>(
                ...
                onMouseOut={e=>{e.currentTarget.style.borderColor="var(--border)"; (e.currentTarget as HTMLElement).style.transform="translateY(0)";}}>
                  <span style={{fontFamily:"var(--font-mono)", fontSize:"13px", color:"var(--accent)", width:"20px", textAlign:"center"}}>{s.icon}</span>
                  <div>
          ...
          <div className="fade-in gold-border" style={{background:"var(--bg)", borderRadius:"8px", padding:"32px"}}>
            {sent ? (
              ...
              <>
                <div style={{fontFamily:"var(--font-mono)", fontSize:"10px", color:"var(--text-muted)", letterSpacing:"0.2em", marginBottom:"24px"}}>SEND A MESSAGE</div>
                <div style={{display:"flex", flexDirection:"column", gap:"16px"}}>
                  {[{key:"name",label:"Your name",type:"text"},{key:"email",label:"Your email",type:"email"}].map(f=>(
                    <div key={f.key}>
                      <label style={{fontFamily:"var(--font-mono)", fontSize:"10px", color:"var(--text-muted)", letterSpacing:"0.12em", display:"block", marginBottom:"6px"}}>{f.label.toUpperCase()}</label>
                      <input type={f.type} value={form[f.key as keyof typeof form]} onChange={e=>setForm({...form,[f.key]:e.target.value})}
                        style={{width:"100%", background:"var(--bg-2)", border:"1px solid var(--border)", borderRadius:"4px", padding:"11px 14px", color:"var(--text)", fontSize:"14px", fontFamily:"var(--font-body)", outline:"none", transition:"border-color 0.2s"}}
                  ...
                  <div>
                    <label style={{fontFamily:"var(--font-mono)", fontSize:"10px", color:"var(--text-muted)", letterSpacing:"0.12em", display:"block", marginBottom:"6px"}}>MESSAGE</label>
                    <textarea rows={5} value={form.message} onChange={e=>setForm({...form,message:e.target.value})}
                      style={{width:"100%", background:"var(--bg-2)", border:"1px solid var(--border)", borderRadius:"4px", padding:"11px 14px", color:"var(--text)", fontSize:"14px", fontFamily:"var(--font-body)", outline:"none", resize:"vertical", transition:"border-color 0.2s"}}
                      onFocus={e=>e.target.style.borderColor="var(--accent)"}
                      onBlur={e=>e.target.style.borderColor="var(--border)"}
                    />
                  </div>
                  <button onClick={()=>setSent(true)} style={{
                    ...
                    border:"none", borderRadius:"4px", cursor:"pointer", fontWeight:600, transition:"opacity 0.2s",
                  }}
        ...
        <div style={{ marginTop:"80px", paddingTop:"28px", borderTop:"1px solid var(--border)", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"12px" }}>
          ...
          <span style={{fontFamily:"var(--font-mono)", fontSize:"11px", color:"var(--text-muted)"}}>Built with <span style={{color:"var(--accent)"}}>Next.js</span> + <span style={{color:"var(--accent)"}}>Tailwind</span></span>
        </div>
      </div>
